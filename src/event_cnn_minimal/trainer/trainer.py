...
class Trainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.val_num_previews = config["trainer"].get(
            "val_num_previews", self.num_previews
        )
        self.val_preview_indices = select_evenly_spaced_elements(
            self.val_num_previews, len(self.valid_data_loader)
        )
            ...
            for loss_ftn in self.loss_ftns:
                ...
                if loss_name == "perceptual_loss":
                    losses[loss_name].append(
                        loss_ftn(pred["image"], image, normalize=True)
                    )
                ...
                if loss_name == "combined_perceptual_loss":
                    losses[loss_name].append(
                        loss_ftn(pred["image"], pred["flow"], image, flow)
                    )
        ...
        for batch_idx, sequence in enumerate(self.data_loader):
            ...
            if batch_idx % self.log_step == 0:
                msg = "Train Epoch: {} {}".format(
                    epoch, self._progress(batch_idx, self.data_loader)
                )
                for k, v in losses.items():
    ...
    def _valid_epoch(self, epoch):
        ...
        for batch_idx, sequence in enumerate(self.valid_data_loader):
            ...
            if batch_idx % self.val_log_step == 0:
                msg = "Valid Epoch: {} {}".format(
                    epoch, self._progress(batch_idx, self.valid_data_loader)
                )
                for k, v in losses.items():
                    msg += " {}: {:.4f}".format(k[:4], v.item())
                self.logger.debug(msg)
            ...
            if (
                batch_idx in self.val_preview_indices
                and (epoch - 1) % self.save_period == 0
            ):
    ...
    def preview(self, sequence, epoch, tag_prefix=""):
        ...
        if self.true_once and tc_loss_ftn is not None:
            for i, image in enumerate(images):
                output = tc_loss_ftn(
                    i, image, pred_images[i], flows[i], output_images=True
                )
                if output is not None:
                    video_tensor = make_tc_vis(output[1])
                    self.writer.writer.add_video(
                        f"warp_vis/tc_{tag_prefix}",
                        video_tensor,
                        global_step=epoch,
                        fps=2,
                    )
        ...
        self.writer.add_histogram(
            f"{tag_prefix}_image/groundtruth", torch.stack(images)
        )
        self.writer.add_histogram(f"{tag_prefix}_input", non_zero_voxel)
        self.writer.add_histogram(
            f"{tag_prefix}_flow/prediction", torch.stack(pred_flows)
        )
        self.writer.add_histogram(
            f"{tag_prefix}_image/prediction", torch.stack(pred_images)
        )
        video_tensor = make_flow_movie(
            event_previews, pred_images, images, pred_flows, flows
        )
        self.writer.writer.add_video(
            f"{tag_prefix}", video_tensor, global_step=epoch, fps=20
        )
