...
class BaseUNet(nn.Module):
    def __init__(
        ...
        self.encoder_input_sizes = [
            int(self.base_num_channels * pow(channel_multiplier, i))
            for i in range(self.num_encoders)
        ]
        self.encoder_output_sizes = [
            int(self.base_num_channels * pow(channel_multiplier, i + 1))
            for i in range(self.num_encoders)
        ]
...
class WNet(BaseUNet):
    ...
    def forward(self, x):
        ...
        for i, decoder in enumerate(self.flow_decoders):
            flow_activations = decoder(
                self.skip_ftn(flow_activations, blocks[self.num_encoders - i - 1])
            )
        image_activations = x
        for i, decoder in enumerate(self.image_decoders):
            image_activations = decoder(
                self.skip_ftn(image_activations, blocks[self.num_encoders - i - 1])
            )
...
class UNet(BaseUNet):
    ...
    def __init__(self, unet_kwargs):
        ...
        for i, (input_size, output_size) in enumerate(
            zip(self.encoder_input_sizes, self.encoder_output_sizes)
        ):
            if i == 0:
