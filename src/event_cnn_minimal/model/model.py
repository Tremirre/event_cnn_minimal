...
from .submodules import ConvGRU, ConvLayer, ResidualBlock
from .unet import UNet, UNetFlow, UNetFlowNoRecur, UNetRecurrent, WNet
...
class ColorNet(BaseModel):
    ...
    def forward(self, event_tensor):
        ...
        image_bgr = merge_channels_into_color_image(
            reconstructions_for_each_channel
        )  # H x W x 3
...
class FireNet(BaseModel):
    ...
    def __init__(self, num_bins=5, base_num_channels=16, kernel_size=3, unet_kwargs={}):
        ...
        if (
            unet_kwargs
        ):  # legacy compatibility - modern config should not have unet_kwargs
        ...
        self.head = ConvLayer(
            self.num_bins, base_num_channels, kernel_size, padding=padding
        )
        ...
        self.pred = ConvLayer(
            base_num_channels, out_channels=1, kernel_size=1, activation=None
        )
